...
st.title("🏥 診所下診時間工具 (v2.8 終極同步修復版)")
...
def check_is_delayed(time_obj, shift_type, clinic_name, date_val=""):
    ...
    if date_val:
        try:
            # 強制用斜線格式解析日期來判斷星期六
            is_sat = datetime.strptime(str(date_val).strip().replace('-','/'), "%Y/%m/%d").weekday() == 5
        except: pass
...
def calculate_time_rule(raw_time_str, shift_type, clinic_name, is_special_morning=False, date_val=""):
    ...
    is_sat = False
    if date_val:
        try: is_sat = datetime.strptime(str(date_val).strip().replace('-','/'), "%Y/%m/%d").weekday() == 5
        except: pass
    is_special_sat = any(c in str(clinic_name) for c in ["立全", "立竹", "上京"])
    ...
    if uploaded_file is not None:
        try:
            ...
            if df is not None:
                ...
                if analysis_file:
                    try:
                        ...
                        if '診所名稱' in df_ana.columns and '日期' in df_ana.columns:
                            ...
                            with c_b: target_dates = st.multiselect("B. 選擇日期 (不選即檢查全月)：", options=date_cols_in_df)
                            ...
                            if st.button("🔍 產生修正預覽", type="primary"):
                                ...
                                for idx, row in df.iterrows():
                                    ...
                                    for col in dates_to_check:
                                        ...
                                        if t_date_key in time_map:
                                            ...
                                            for s_type in ["早", "午", "晚"]:
                                                if s_type in shifts:
                                                    st_t = {"早": "08:00", "午": "15:00", "晚": "18:30"}[s_type]
                                                    if is_licheng and s_type == "午": st_t = "14:00"
                                                    if is_licheng and s_type == "晚": st_t = "18:00"
                                                    
                                                    ed_t = {"早": "12:00", "午": "18:00", "晚": "21:30"}[s_type]
                                                    if is_special and s_type == "早": ed_t = "13:00"
                                                    if is_licheng and s_type == "午": ed_t = "17:00"
                                                    if is_licheng and s_type == "晚": ed_t = "21:00"

                                                    orig_t_str = vals.get(s_type)
                                                    if pd.notna(orig_t_str) and str(orig_t_str).strip().lower() != 'nan':
                                                        t_obj = parse_time_obj(orig_t_str)
                                                        if t_obj:
                                                            is_d, _ = check_is_delayed(t_obj, s_type, selected_clinic, t_date_key)
                                                            if is_d:
                                                                has_any_delay = True
                                                                fixed_ed = calculate_time_rule(orig_t_str, s_type, selected_clinic, is_special, t_date_key)
                                                                if fixed_ed: ed_t = fixed_ed
                                                    
                                                    shift_times.append(f"{st_t}{selected_conn}{ed_t}")
                                ...
                                if changes_list:
                                    ...
                                    st.success(f"找到 {len(changes_list)} 筆資料可更新。(醫師/店長/主管預設不勾選)")
                                else: 
                                    st.warning("比對完畢。所有人員皆準時完診，無須更新。請確認上傳的【完診分析】內是否有該診所資料。")
                ...
                with c_btn1: sta_code = st.text_input("例假日代號", "{sta}")
                with c_btn2: res_code = st.text_input("休息日代號", "{res}")
                with c_btn3:
                    st.write("")
            ...
            if st.session_state.working_df is not None:
                ...
                with c1: st.download_button(f"📥 下載 Excel 匯入檔", data_export, '排班表_回填完成.xlsx', type="primary")
                with c2: 
                    try: st.download_button("📥 下載 Big5 CSV", df_export.to_csv(index=False, encoding='cp950', errors='replace', quoting=csv.QUOTE_ALL), '排班表_Big5.csv', 'text/csv')
                    except: pass
                with c3: st.download_button("📥 下載 UTF-8 CSV", df_export.to_csv(index=False, encoding='utf-8-sig'), '排班表_UTF8.csv', 'text/csv')

        except Exception as e: st.error(f"發生錯誤: {e}")

# ==========================================
# 分頁 2: 完診分析 (100% 保留原始對照表格式與延診預覽)
# ==========================================
with tab2:
    ...
    if upl:
        ...
        try:
            ...
            idx_s = next((i for i, x in enumerate(cols) if any(k in x for k in ["班", "時段"])), 1 if len(cols)>1 else 0)
            idx_t = next((i for i, x in enumerate(cols) if any(k in x for k in ["完診", "時間"])), len(cols)-1)
            ...
            if st.button("🚀 開始分析並偵測延診", key="an_btn"):
                ...
                if res:
                    ...
                    shifts.sort(key=sk); final = final[base + shifts].fillna("").sort_values(by=d_c)
                    ...
                    for _, row in final.iterrows():
                        clinic = row['診所名稱']; date_val = row[d_c]; raw_m = str(row[col_m]) if col_m else ""; raw_a = str(row[col_a]) if col_a else ""; raw_e = str(row[col_e]) if col_e else ""
                        fix_m, fix_a, fix_e = "", "", ""
                        if raw_m:
                            t = parse_time_obj(raw_m)
                            if t:
                                is_d, lim = check_is_delayed(t, "早", clinic, date_val)
                                if is_d: delayed_records.append({"日期": date_val, "診所": clinic, "班別": "早", "標準": lim, "實際": t.strftime("%H:%M")})
                                fix_m = calculate_time_rule(raw_m, "早", clinic, False, date_val)
                        if raw_a:
                            t = parse_time_obj(raw_a)
                            if t:
                                is_d, lim = check_is_delayed(t, "午", clinic, date_val)
                                if is_d: delayed_records.append({"日期": date_val, "診所": clinic, "班別": "午", "標準": lim, "實際": t.strftime("%H:%M")})
                                fix_a = calculate_time_rule(raw_a, "午", clinic, False, date_val)
                        if raw_e:
                            t = parse_time_obj(raw_e)
                            if t:
                                is_d, lim = check_is_delayed(t, "晚", clinic, date_val)
                                if is_d: delayed_records.append({"日期": date_val, "診所": clinic, "班別": "晚", "標準": lim, "實際": t.strftime("%H:%M")})
                                fix_e = calculate_time_rule(raw_e, "晚", clinic, False, date_val)

                        export_rows.append({"診所名稱": clinic, "日期": date_val, "早上(原始)": raw_m, "早上": fix_m, "下午(原始)": raw_a, "下午": fix_a, "晚上(原始)": raw_e, "晚上": fix_e})
                    ...
                    st.subheader("📥 下載對照報表")
                    o = io.BytesIO()
                    with pd.ExcelWriter(o, engine='openpyxl') as w: df_export_final.to_excel(w, index=False)
                    st.download_button("📥 下載完診分析結果 (.xlsx)", o.getvalue(), "完診分析結果_對照檔.xlsx", type="primary")
        except Exception as e: st.error(f"發生錯誤: {e}")
