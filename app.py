...
def ultimate_clean(val):
    ...
    # 專武殺手：無情剿滅 Femas 產生的佔位時間 "00:00-00:00" 以及附帶的診所代碼
    s = re.sub(r'[,\s\n;]*00:00-00:00[,\s\n;]*[^\s,;]*', '', s)

    # 1. 根除正方形與三角形
    s = re.sub(r'[■□▲△]', '', s)
    
    # 2. 如果裡面沒有中文字、英文字母、數字或大括號，直接判定為無效內容，回傳乾淨空白
...
def calculate_time_rule(raw_time_str, shift_type, clinic_name, is_special_morning=False, date_str=""):
    ...
    if shift_type == "早":
        ...
        if is_licheng:
            ...
            if is_saturday and is_special_sat_clinic:
                ...
                with st.expander("⚙️ 欄位與人員設定 (純早班調整)", expanded=False):
                    ...
                    if id_col != "(不修正)":
                        def fix_id(val):
                            ...
                            if v_str.lower() == 'nan' or not v_str: return ""
                            # 🚀 智慧編號補零：如果是純數字才補四碼，P067 這種英文開頭的就原封不動
                            return v_str.zfill(4) if v_str.isdigit() else v_str
                            
                        df[id_col] = df[id_col].apply(fix_id)
                                    ...
                                    for col in dates_to_check:
                                        ...
                                        if t_date in time_map:
                                            ...
                                            if cell_val and cell_val.lower()!='nan':
                                                ...
                                                # 📍 微創手術 3：改寫字串拼湊邏輯，套用您指定的文字標籤
                                                formatted_segments = []
                                                has_afternoon = "午" in shifts
                                                has_evening = "晚" in shifts

                                                if "早" in shifts:
                                                    start_t = "08:00"
                                                    end_t = "13:00" if is_special else "12:00"
                                                    orig_t_str = vals.get("早")
                                                    if pd.notna(orig_t_str) and str(orig_t_str).strip().lower() != 'nan':
                                                        ...
                                                        if t_obj:
                                                            ...
                                                            if is_d:
                                                                has_delay = True
                                                                fixed_t_str = calculate_time_rule(orig_t_str, "早", selected_clinic, is_special, col)
                                                                if fixed_t_str: end_t = fixed_t_str
                                                    formatted_segments.append(f"{start_t}{selected_conn}{end_t} 早班")

                                                if has_afternoon and has_evening:
                                                    start_t = "14:00" if is_licheng else "15:00"
                                                    end_t = "21:00" if is_licheng else "21:30"
                                                    orig_t_str = vals.get("晚")
                                                    if pd.notna(orig_t_str) and str(orig_t_str).strip().lower() != 'nan':
                                                        t_obj = parse_time_obj(orig_t_str)
                                                        if t_obj:
                                                            ...
                                                            if is_d:
                                                                has_delay = True
                                                                fixed_t_str = calculate_time_rule(orig_t_str, "晚", selected_clinic, False, col)
                                                                if fixed_t_str: end_t = fixed_t_str
                                                    formatted_segments.append(f"{start_t}{selected_conn}{end_t} 午晚班一起")

                                                elif has_afternoon:
                                                    start_t = "14:00" if is_licheng else "15:00"
                                                    end_t = "17:00" if is_licheng else "18:00"
                                                    orig_t_str = vals.get("午")
                                                    if pd.notna(orig_t_str) and str(orig_t_str).strip().lower() != 'nan':
                                                        t_obj = parse_time_obj(orig_t_str)
                                                        if t_obj:
                                                            ...
                                                            if is_d:
                                                                has_delay = True
                                                                fixed_t_str = calculate_time_rule(orig_t_str, "午", selected_clinic, False, col)
                                                                if fixed_t_str: end_t = fixed_t_str
                                                    formatted_segments.append(f"{start_t}{selected_conn}{end_t} 午班")

                                                elif has_evening:
                                                    start_t = "18:00" if is_licheng else "18:30"
                                                    end_t = "21:00" if is_licheng else "21:30"
                                                    orig_t_str = vals.get("晚")
                                                    if pd.notna(orig_t_str) and str(orig_t_str).strip().lower() != 'nan':
                                                        t_obj = parse_time_obj(orig_t_str)
                                                        if t_obj:
                                                            ...
                                                            if is_d:
                                                                ...
                                                                fixed_t_str = calculate_time_rule(orig_t_str, "晚", selected_clinic, False, col)
                                                                if fixed_t_str: end_t = fixed_t_str
                                                    formatted_segments.append(f"{start_t}{selected_conn}{end_t} 晚班")

                                                if has_delay:
                                                    final_val = selected_sep.join(formatted_segments)
...
with tab2:
    ...
    if upl:
        ...
        try:
            ...
            # 📍 微創手術 4：極度嚴謹的欄位自動防呆偵測
            idx_d = next((i for i, x in enumerate(cols) if "日期" in x), 0)
            
            # 確保時段別絕對不會跟日期重複
            idx_s_candidates = [i for i, x in enumerate(cols) if i != idx_d and any(k in x for k in ["班", "午", "時段"])]
            idx_s = idx_s_candidates[0] if idx_s_candidates else (1 if len(cols)>1 else 0)
            
            # 確保時間絕對不會跟日期或時段別重複
            idx_t_candidates = [i for i, x in enumerate(cols) if i != idx_d and i != idx_s and any(k in x for k in ["時間", "完診", "下診"])]
            idx_t = idx_t_candidates[0] if idx_t_candidates else (len(cols)-1)
            ...
            if st.button("🚀 開始分析並偵測延診", key="an_btn"):
                ...
                if res:
                    ...
                    for idx, row in final.iterrows():
                        ...
                        if raw_m and raw_m.lower()!='nan':
                            ...
                            if t:
                                ...
                                fix_m = calculate_time_rule(raw_m, "早", clinic, False, date_val) or raw_m
                        ...
                        if raw_a and raw_a.lower()!='nan':
                            ...
                            if t:
                                ...
                                fix_a = calculate_time_rule(raw_a, "午", clinic, False, date_val) or raw_a
                        ...
                        if raw_e and raw_e.lower()!='nan':
                            ...
                            if t:
                                ...
                                fix_e = calculate_time_rule(raw_e, "晚", clinic, False, date_val) or raw_e
                    ...
                    def highlight_delay_rows(row):
                        ...
                        clinic = str(row['診所名稱'])
                        date_val_str = str(row['日期'])
                        
                        def apply_yellow(val_str, shift_type):
                            if val_str:
                                t = parse_time_obj(val_str)
                                is_d, _ = check_is_delayed(t, shift_type, clinic, date_val_str)
