                                                ...
                                                for s in ["早", "午", "晚"]:
                                                    if s in shifts:
                                                        ...
                                                        if pd.notna(orig_t_str) and str(orig_t_str).strip().lower() != 'nan':
                                                            ...
                                                            if t_obj:
                                                                is_d, _ = check_is_delayed(t_obj, s, selected_clinic, col)
                                                                if is_d:
                                                                    has_delay = True
                                                                    fixed_t_str = calculate_time_rule(orig_t_str, s, selected_clinic, is_special, col)
                                                                    if fixed_t_str:
                                                                        end_t = fixed_t_str
...
with tab2:
    ...
    if upl:
        ...
        try:
            ...
            # 🎯 恢復原本的欄位抓取方式，微調「時段」避開「時間」
            idx_d = next((i for i, x in enumerate(cols) if "日期" in x), 0)
            idx_s = next((i for i, x in enumerate(cols) if any(k in x for k in ["午", "班", "時段"])), 1 if len(cols)>1 else 0)
