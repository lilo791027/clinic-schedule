                        ...
                        if '診所名稱' in df_ana.columns and '日期' in df_ana.columns:
                            ...
                            if st.button("🔍 產生修正預覽", type="primary"):
                                ...
                                if changes_list:
                                    ...
                                    st.success(f"找到 {len(changes_list)} 筆更新。(醫師/主管/店長預設不勾選)")
        ...
        try:
            ...
            if st.button("🚀 開始分析", key="an_btn"):
                ...
                if res:
                    ...
                    def highlight_delay_rows(row):
                        ...
                        def apply_yellow(val_str, shift_type):
                            if val_str:
                                ...
                                is_d, _ = check_is_delayed(t, shift_type, clinic, date_str_val)
                    ...
                    st.download_button("📥 下載完整分析報表 (.xlsx)", data=o.getvalue(), file_name='完診分析報表.xlsx', mime='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet', type="primary")
        except Exception as e: st.error(f"錯誤: {e}")
